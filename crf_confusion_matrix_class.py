...
class crf_confusion_matrix_class:
		...
		while(1):
			...
			if not line:
				...
				if (line.replace("\n","")!=""):
					...
					self.guess_all.append(data_raw)
	...
	def crf_result_correct_and_wrong_file_mapping(self):
		...
		f_guess_all = open("f_guess_all", "w")
		for i in range(0,len(self.guess_all)):
			f_guess_all.write(str(self.guess_all[i]) + "\n")
		f_guess_all.close()
	...
	def confusion_matrix_print_func(self):
		...
		self.crf_result_correct_and_wrong_file_mapping()
